# data-structure
 数据结构算法整理

# 已实现
## 字符串
1. KMP
2. AC Trie

## 二叉树
1. 前序遍历、中序遍历、后序遍历
2. Morris遍历
3. 二叉搜索树

## LeetCode题目
1. 两数之和：利用字典
2. 链表反转


# 未实现
## 查找第k小（大）的数：
1. 二分法
2. bfptr    O(N)
## 利用KMP：
1. 判断是否是翻转字符串
2. 判断一棵树是否是另一棵树的子树：利用先序遍历得到节点数组，利用KMP判断是否为子数组
## 查找同源异构体
利用负债表，窗口滑动查找

## 并查集
bool isSameSet(x, y)    O(1)
union(x, y)             O(1)
1. GetUser, self.a, self.b, self.c

## 图
1. 构造经典图表示
2. 其他图表示转换为经典表示
3. 图的广度优先遍历
4. 图的深度优先遍历
5. 图的拓扑排序（有向无环图）
6. 最小生成树   
- Kruskal算法：利用并查集
- Prim算法：先激活点，再激活边，找最小激活边，激活点，依次循环...
## 排序
1. 选择排序
2. 冒泡排序
3. 插入排序
## 二分法
1. 有序数组，是否包含元素m
2. 有序数组，找到<=m的最右位置
3. 有序数组，找到>=m的最左位置
4. 找局部最小值/相邻最小，利用数学单调性
## 异或运算
- 二进制运算无进位相加
- 满足交换律和结合律
- 0^N=N
- N^N=0
- a^b^c^d^e=a^d^b^c=... 与位置无关
1. 交换两个整数值，不申请额外变量（必须是两个不同的变量，如果是同一个变量，最后会=0）【不实用，纯装X】
- int a=甲, int b=乙
- a = a^b（甲^乙）
- b = a^b（甲^乙^乙=甲）
- a = a^b（甲^乙^甲=乙）
2. 数组中有一个数出现了奇数次，其他数都出现了偶数次
    - 把所有数异或起来，最后的结果就是这个奇数次出现的数
3. int a=N，求N的二进制最右一位1
    - rightOne = N&(~N+1)
    - 举例：N=0101001000，rightOne=0000001000
4. 数组中有两个数出现了奇数次，其他数都出现了偶数次
    - 把所有数异或起来，得到 eor = A^B
    - rightOne = eor & (~eor+1)
    - 利用rightOne将数组分为两部分，一部分求&=0（P1），另一部分求&!=0（P2）
    - 分别对P1,P2求异或，得到A和B（可以求出一个之后利用eor^求出另一个）
5. 求一个数的二进制出现了多少个1
    - 先求rightOne, +1
    - ^rightOne, 再求rightOne, +1
    - 继续直到=0

## 链表
*技巧：*
- 使用容器
- 快慢指针（减少空间复杂度）
1. 输入链表头结点，奇数长度返回中点，偶数长度返回上中点（已完成）

2. 输入链表头结点，奇数长度返回中点，偶数长度返回下中点（已完成）

3. 输入链表头结点，奇数长度返回中点前一个，偶数长度返回上中点前一个（已完成）

4. 输入链表头结点，奇数长度返回中点前一个，偶数长度返回下中点前一个（已完成）

5. 给定一个单链表，判断是否为回文链表
- 5.1 利用栈（base)
- 5.2 利用快慢指针得到中点/上中点，将中点/上中点之后的部分加入栈，从头比较(base1)
- 5.3 利用快慢指针得到中点/上中点，将中点/上中点指向None，后续节点反向，从头部和尾部开始比较，当左边指向None时停止，期间有不同就返回False；判断完之后将后半部分逆序回来(optimize)

6. 将单链表按照某值划分成左边小，中间相等，右边大的形式
- 6.1 利用数组，将节点放进去，按照值partition，按顺序重组链表（base）
- 6.2 用三组头尾节点标识符（小于区域头、尾；等于区域头尾；大于区域头尾），遍历链表每个节点，小于值的放入小于区域，等于值的放入等于区域，大于值的放入大于区域，最后，将小于区域的尾连等于区域的头，等于区域的尾连大于区域的头（考虑小于区域等于区域大于区域可能为空的情形，coding注意）

7. 特殊单链表（含rand指针），复制链表，时间复杂度O(N),空间复杂度O(1)
- 7.1 利用hashmap，存储老新指针映射关系，利用hashmap将新节点的next指针和rand指针指向对应的新节点
- 7.2 不使用hashmap
    - copy节点：遍历节点，克隆每一个节点连在自身的后面；
    - 连接rand：每次拿出一对老新节点，将新节点的rand指针指向老节点rand的后一个节点(clone节点)；
    - 拆分：首先记录新节点的head，然后每次拿出一对老新节点，老指向老，新指向新，返回head

8. 判断单链表是否有环，若有环，返回第一个入环的节点，若无环，返回None
- 8.1 判断是否有环
    - 利用快慢指针，初始化，慢指针指向head.next，快指针指向head.next.next(防止退出while循环)
    - 慢指针走一步，快指针走两步，由于快指针先入环，慢指针后入环，入环后，两指针终将相遇
        - 证明：
        - 假设慢指针走了n步走到入环的第一个节点（则此时快指针走了2n步），并假设此时快指针在距离慢指针m步的位置（假定慢指针不动，快指针走m步可到达慢指针），则第 2n+m 步会到达入环第一个节点（后续8.2求node有用）
        - 设此时再走x次两指针相遇（由于两者在一个环上因此必相遇），则有 2x - m = x（快指针比慢指针多走m步）
        - 求得 x = m，即再走m步两指针必定相遇
        - 并可以得到，相遇时慢指针走了 n+m 步
- 8.2 求入环第一个节点
    - 两指针第一次相遇后，慢指针不动，快指针回到head，两指针同时往前走一步，直到相遇，相遇时即在入环第一个节点
    - 证明：
    - 两指针第一次相遇时，慢指针走了 n+m 步，快指针从head开始，走n步后到达入环第一个节点，此时慢指针走了 n+m+n=2n+m 步，正好也到达入环第一个节点（见8.1证明），因此得证

9. 约瑟夫环问题


## 二叉树
0. 序列化和反序列化

1. 打印二叉树
    右中左递归遍历打印节点，记录层数，利用层数填充空格，用符号表示节点属性（左、右）

2. 二叉树（有父亲节），求后继节点（中序遍历的后一个节点）
    - 若有右节点，则后继节点为右节点的最左子节点
    - 若无右节点，则找父节点，直到自己等于父节点的左节点，此时的父节点为后继节点

3. 折纸条，打印折痕
    - 对折一次：一号折痕为凹
    - 对折两次：一号折痕上面的二号折痕为凹，下面的二号折痕为凸
    - 对折三次：二号折痕上面增加三号折痕凹，下面增加三号折痕凸
    - 以此类推，用中序遍历的方式打印折痕

4. 二叉树的递归套路
- 4.1 平衡树：每一颗子树，左树和右树高度差不超过1
- 4.2 求节点的最大距离
    递归求：左子树的最大距离、右子树的最大距离、左子树距离自己最大的距离和右子树距离自己最大的距离中较大的距离，取三者最大
- 4.3 求最大搜索二叉子树的头结点（或者节点个数）
- 4.5 派对最大快乐值
    每个员工有最大快乐值和直接下属
    若给某个员工发请柬，TA一定来，TA的直接下属一定不来
    求整棵树的最大快乐值
    - 某个员工，如果来，则该子树的最大快乐值=员工快乐值+所有直接员工不来的子树最大快乐值
    - 如果不来，则该子树的最大快乐值=求和（所有直接员工：max(来、不来的子树最大快乐值))

## 暴力递归
### 从左往右的尝试模型
1. 打印字符串的所有子序列
    「子串：连续的子串
      子序列：不必连续，顺序相同」

2. 打印字符串的所有字面不重复子序列
    - 在题1中的使用集合存储结果

3. 打印字符串的所有全排列，字符串长度为n
    - 第一个位置可能有n种情况
    - 第i个位置可能有n-i+1种情况
    - 递归每种情况

4. 打印字符串的所有不重复全排列（已实现）
    - 4.1 用set存储结果
    - 4.2 对于每个位置，若已经出现过某个字符，则弃掉该分支（分支限界）

5. 数字组成的字符串映射为字母，1~26对应A~Z，求所有可能映射结果或结果个数(递归法已实现)
    - 例：111可以映射为A或者AK，KA
    - 考虑每个位置可能的映射结果，分单独映射和结合下一位数映射
    - 边界条件：
        过了结束的位置，说明映射结束，返回映射结果或者映射结果数+1
        当前位置数字为0，则当前路径无映射可能，无映射结果或者映射结果数+0
    - 每个位置的可能性分情况讨论：
        若该位置数字为1，则分单独映射和结合下一位数映射（当下一个位置没有超过字符串长度）
        若该位置数字为2，则分单独映射和结合下一位数映射（下一位置不超过字符串且下一数字小于7）
        若该位置数字大于2，则只可能是单独映射

6. 背包具有价值和重量，选择背包使得满足重量不超bag的情况下价值最大，返回最大价值(已实现)
    - 6.1 后向前传递
        - 边界条件：
            背包已处理完，返回价值0
            当前的总重量已超过阈值，此路径方案不通
        - 当前背包可以选择要或者不要
        - 递归获取后续的背包选择出来的最大价值
        - 返回要或不要两种情况的较大价值
    - 6.2 剩余重量
        - 初始化：第1个背包，剩余重量为bag
        - 边界条件：达到第n个背包
        - 返回当前背包要或者不要的情况下，价值的最大值
        - 剩余重量大于0时递归调用
    - 6.3 以上两思路动态规划实现

7. N皇后问题：棋盘上放N个皇后，每个皇后不同行、不同列、不斜线（复杂度：N的N次方）
    - 7.1 暴力递归
        - 前 i-1 行都放置好之后，第i行遍历每个位置j判断是否满足和前i-1个不同列、不斜线
        - 若第1个位置在k列，则合法判断：j!=k; 不斜线：|i-1|！=|j-k|
        - 若遍历到第N行且合法，则满足条件的结果+1
    - 7.2 位运算暴力递归(减少复杂度里的常数项)
        - 用二进制后N位为1其余位置为0的数表示目标结果limit
        - 用三个变量表示列限制、左限制和右限制
        - 初始化:列限制、左限制、右限制均为0
        - 每放置一个皇后，列限制在该位置上+1，左限制先在该位置上+1然后向左位运算移一位，右限制先在该位置上+1然后向右位运算移一位
        - 每次放置前，计算可放置位置pos：~（列限制|左限制|右限制）
        - 对于pos，每次找二进制最右的1（pos & (~pos+1)），在该位置上放置皇后
        - 若列限制==limit，则返回一种结果

## 动态规划
### 【不是所有暴力递归都能改成动态规划，所有的动态规划都来自于暴力递归，动态规划是用来减少暴力递归的重复过程】
### 记忆化搜索和动态规划的粒度不同，动态规划的粒度更细
1. 有N个节点，起始点为S，走K步，走到P点的方式有几种（walk）
    - 1.1 暴力递归
    - 1.2 做缓存表（用矩阵或者hash表）
    - 1.3 动态规划

2. 博弈论（Game Theory）
    - 题目
        - 有一个点数序列，每次可以从左边或者右边取一个数
        - A、B两个人一人取一个数，最后算各自取数的和，较大的获胜
        - 假定A、B都会选择最有利自己的拿法，求先拿还是后拿获胜
    - 题解
        - 两个函数：先手拿的最大和，后手拿最小和

3. 有一个数组，表示钞票的面额，可以任意选择任意张面额的钞票，要刚好拿到总额=object，有多少种选择方法（money choice）
    - 临界条件：选择完所有面额时，若余额rest=0，则返回一种选择
    - 在选择第i种面额s时，有rest//s种选择
    - 
